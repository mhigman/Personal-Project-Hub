...
export let validMessage = false;
export function setSocketListeners() {
    const ws = WSClient;
    ws.open("ws://localhost:8080/socket"); 
    ws.onopen = () => {
        console.log("connection online");
    };
    ws.onclose = () => {
        console.log("connecntion terenmentsationastt");
    };
    ws.onerror = () => {
    ...
    ws.onmessage = (e) => {
        ...
        document.querySelector("#chat").value += message["username"] + " : " + message["message"] + "\r\n"
        validMessage = true;
    }
    return ws;
}